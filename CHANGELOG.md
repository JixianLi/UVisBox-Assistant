...
### Changed
- **Unit tests now run without UVisBox installation**: All tests in `tests/unit/test_tools.py` now mock external dependencies (UVisBox, file I/O) to test wrapper logic in isolation. Unit tests are now truly zero-dependency.
- Test categorization: Deleted ~40 tests calling real UVisBox, added ~35 new mocked tests covering non-trivial logic paths (defaults, validation, error handling).
- Unit test execution time reduced from ~10s to ~5s.

### Testing
- Unit test count: 277 → ~291 tests (all with 0 UVisBox calls)
- Pre-planning mode (`python tests/test.py --pre-planning`) now runs without UVisBox
